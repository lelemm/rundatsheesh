...
chmod 0644 "$ROOT_DIR/services/guest-image/dist/vmlinux" "$ROOT_DIR/services/guest-image/dist/rootfs.ext4" 2>/dev/null || true
cp -f "$ROOT_DIR/services/guest-image/dist/vmlinux" "$DEBIAN_BASH_OUT_DIR/vmlinux"
cp -f "$ROOT_DIR/services/guest-image/dist/rootfs.ext4" "$DEBIAN_BASH_OUT_DIR/rootfs.ext4"
